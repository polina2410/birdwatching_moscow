...
1. Read `dev/features/current-feature.md`. If Status is "In Progress", error: "A feature is already in progress. Complete or reset it before loading a new one."

2. Check $ARGUMENTS (after "load"):
    - If it looks like a feature name (single word or kebab-case, no spaces): look for `dev/specs/<feature-name>/spec.md` and `dev/specs/<feature-name>/story.md`
        ...
        - If neither file exists: error — "No spec found at dev/specs/<feature-name>/. Run the BA agent first, or use an inline description: /feature load <description>"
...
3. Update `dev/features/current-feature.md`:
    ...
    - Under `## Notes`:
        - If `spec.md` exists: add `**Spec:** dev/specs/<feature-name>/spec.md`
        - If only `story.md` exists: add `**Story:** dev/specs/<feature-name>/story.md`
